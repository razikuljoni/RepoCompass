...
function parseRepository(raw: string) {
  ...
  if (url.hostname === "github.com" && parts.length >= 2)
    return { provider: "GitHub", owner: parts[0], name: parts[1], key: `${parts[0]}/${parts[1]}` };
  ...
  if (url.hostname === "bitbucket.org" && parts.length >= 2)
    return {
      provider: "Bitbucket",
      owner: parts[0],
      name: parts[1],
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (repo.provider === "GitHub") {
      const infoResponse = await fetch(`https://api.github.com/repos/${repo.key}`, {
        headers: { Accept: "application/vnd.github+json", "User-Agent": "RepoCompass" },
      });
      if (!infoResponse.ok)
        throw new Error(
          infoResponse.status === 404
            ? "Repository not found or not public."
            : `GitHub returned ${infoResponse.status}. Try again shortly.`,
        );
      ...
      const [branchesResponse, commitsResponse] = await Promise.all([
        fetch(`https://api.github.com/repos/${repo.key}/branches?per_page=100`, {
          headers: { Accept: "application/vnd.github+json", "User-Agent": "RepoCompass" },
        }),
        fetch(`https://api.github.com/repos/${repo.key}/commits?per_page=20`, {
          headers: { Accept: "application/vnd.github+json", "User-Agent": "RepoCompass" },
        }),
      ]);
      if (branchesResponse.ok)
        branches = ((await branchesResponse.json()) as Array<{ name: string }>).map((x) => x.name);
      ...
      const treeResponse = await fetch(
        `https://api.github.com/repos/${repo.key}/git/trees/${encodeURIComponent(branch)}?recursive=1`,
        { headers: { Accept: "application/vnd.github+json", "User-Agent": "RepoCompass" } },
      );
      ...
      files = tree.tree.reduce<RemoteFile[]>((result, item) => {
        if (item.type === "blob")
          result.push({ path: item.path, size: item.size, type: item.type });
        return result;
      }, []);
      ...
      if (!infoResponse.ok)
        throw new Error(
          infoResponse.status === 404
            ? "Repository not found or not public."
            : `GitLab returned ${infoResponse.status}.`,
        );
      const info = (await infoResponse.json()) as {
        default_branch: string;
        description?: string;
      ...
      if (!infoResponse.ok)
        throw new Error(
          infoResponse.status === 404
            ? "Repository not found or not public."
            : `Bitbucket returned ${infoResponse.status}.`,
        );
      const info = (await infoResponse.json()) as {
        mainbranch?: { name: string };
        description?: string;
      };
      branch = info.mainbranch?.name ?? "main";
      description = info.description ?? "";
      const branchesResponse = await fetch(
        `https://api.bitbucket.org/2.0/repositories/${repo.key}/refs/branches?pagelen=100`,
      );
      if (branchesResponse.ok)
        branches = (
          (await branchesResponse.json()) as { values: Array<{ name: string }> }
        ).values.map((x) => x.name);
      const treeResponse = await fetch(
        `https://api.bitbucket.org/2.0/repositories/${repo.key}/src/${encodeURIComponent(branch)}/?pagelen=100&max_depth=10`,
      );
      if (!treeResponse.ok) throw new Error("Could not read the repository tree.");
      const tree = (await treeResponse.json()) as {
    ...
    for (const file of accepted) {
      const extension = file.path.split(".").at(-1)?.toLowerCase() ?? "other";
    ...
    const contentCandidates = accepted
      .filter(
        (file) =>
          textExtensions.has(file.path.split(".").at(-1)?.toLowerCase() || "") &&
          (file.size || 0) <= 120_000,
      )
      .slice(0, 80);
    const indexedFiles: IndexedFile[] = await Promise.all(
      contentCandidates.map(async (file) => {
        try {
          let raw = "";
          if (repo.provider === "GitHub")
            raw = `https://raw.githubusercontent.com/${repo.key}/${encodeURIComponent(branch)}/${file.path.split("/").map(encodeURIComponent).join("/")}`;
          else if (repo.provider === "GitLab")
            raw = `https://gitlab.com/api/v4/projects/${encodeURIComponent(repo.key)}/repository/files/${encodeURIComponent(file.path)}/raw?ref=${encodeURIComponent(branch)}`;
          else
            raw = `https://api.bitbucket.org/2.0/repositories/${repo.key}/src/${encodeURIComponent(branch)}/${file.path.split("/").map(encodeURIComponent).join("/")}`;
    ...
    return NextResponse.json({
      ...
      ignored: excluded,
      bytes: accepted.reduce((sum, file) => sum + (file.size ?? 0), 0),
      languages,
